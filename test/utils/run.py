...
def main():
    ...
    # entire group if it exceeds GPULIB_TEST_TIMEOUT seconds (default 30, 0 to
    # disable). Killing the group (not just the child) is required to tear down
    # the GPU context and the persistent scheduler kernel. A timed-out test
    # returns non-zero, which lit records as a normal FAIL/XFAIL instead of a hang.
    try:
        timeout = float(os.environ.get("GPULIB_TEST_TIMEOUT", "30"))
    except ValueError:
        timeout = 30.0
